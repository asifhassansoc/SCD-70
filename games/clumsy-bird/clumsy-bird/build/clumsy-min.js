...
function showimg(resimg) {
  ...
  <img src="${resimg}" onclick="resize('${resimg}')" data-dismiss="modal" class="card-img-top" width="300px" height="100px" />
</div>
</div>`
}

function resize(img) {
  game.resources[1].src = img;
  hide();
...
function clumsyscore(save) {
  ...
  fetch("http://localhost:8081/score", {
    ...
    method: "POST",
    body: JSON.stringify({ "ethadd": ethadd, "score": score, }),
  })
};
